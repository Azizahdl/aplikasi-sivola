...
<div class="container-fluid">
    <div class="materi-card">
            ...
            <div class="filter-row d-flex align-items-center gap-2 flex-wrap mt-3">
                <div class="filter-btns">
                    ...
                    <a href="{{ route('guru.materi.index', array_merge(request()->except('kategori'), ['kategori' => 'suku_kata', 'search' => request('search')])) }}"
                       class="btn-filter {{ request('kategori') == 'suku_kata' ? 'active f-green' : '' }}">Suku Kata</a>
                    <a href="{{ route('guru.materi.index', array_merge(request()->except('kategori'), ['kategori' => 'kata_dasar', 'search' => request('search')])) }}"
                       class="btn-filter {{ request('kategori') == 'kata_dasar' ? 'active f-orange' : '' }}">Kata Dasar</a>
                </div>
                <form method="GET" action="{{ route('guru.materi.index') }}" class="ms-auto">
                    ...
                    <div class="search-pill">
                        ...
                        <input type="text" name="search" placeholder="Cari teks bacaan..." value="{{ request('search') }}">
        ...
        <div class="show-row d-flex align-items-center gap-2">
            Tampilkan
            <select onchange="location.href='?per_page='+this.value+'&kategori={{ request('kategori') }}&search={{ request('search') }}'">
        ...
        <div class="table-responsive">
            <table class="table-custom">
                <thead>
                    <tr>
                        <th class="tc" style="width:44px;">No</th>
                        <th>Teks Bacaan</th>
                        <th class="tc" style="width:150px;">kategori Materi</th>
                        {{-- <th class="tc" style="width:110px;">Threshold</th> --}}
                ...
                <tbody>
                    @forelse($materi as $index => $item)
                        @php
                            $kategoriColors = ['abjad'=>'b-blue','suku_kata'=>'b-green','kata_dasar'=>'b-orange'];
                            $colorClass = $kategoriColors[$item->kategori] ?? 'b-blue';
                        @endphp
                        <tr>
                            <td class="tc no-cell">{{ $materi->firstItem() + $index }}</td>
                            <td style="font-weight:500;">{{ Str::limit($item->teks_bacaan, 70) }}</td>
                            <td class="tc">
                            ...
                            <td class="tc">
                                <div class="aksi-wrap">
                                    ...
                                    <form action="{{ route('guru.materi.destroy', $item->id_materi) }}"
                            ...
                            <td colspan="5">
                                <div class="empty-state">
                                    <svg width="36" height="36" fill="none" viewBox="0 0 24 24">
                                        <path d="M4 19.5A2.5 2.5 0 016.5 17H20" stroke="#b0a89e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z" stroke="#b0a89e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
        ...
        <form action="{{ route('guru.materi.store') }}" method="POST">
            ...
            <div class="modal-body">
                <div class="form-group-m">
                    ...
                    @error('teks_bacaan')<div class="error-m">{{ $message }}</div>@enderror
                </div>
                <div class="form-row-m">
                    <div class="form-group-m mb-0">
                        <label class="form-label-m" for="kategori">kategori Materi <span class="req">*</span></label>
                        ...
                        @error('kategori')<div class="error-m">{{ $message }}</div>@enderror
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-modal-save">
                    <svg width="13" height="13" fill="none" viewBox="0 0 16 16">
                        <path d="M2 8.5L6 12.5L14 4" stroke="#fff" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    Simpan
                </button>
            </div>
        ...
        <div class="modal-body">
            ...
            <div class="rekam-status" id="rs-idle">
                <svg width="40" height="40" fill="none" viewBox="0 0 24 24">
                    <rect x="9" y="2" width="6" height="12" rx="3" stroke="#8a96a8" stroke-width="1.5"/>
                    <path d="M5 10a7 7 0 0014 0M12 19v3M8 22h8" stroke="#8a96a8" stroke-width="1.5" stroke-linecap="round"/>
                </svg>
            ...
            <div class="rekam-status" id="rs-recording" style="display:none">
                <div class="rec-pulse">
                    <svg width="22" height="22" fill="none" viewBox="0 0 24 24">
                        <rect x="9" y="2" width="6" height="12" rx="3" fill="#ef4444" stroke="#ef4444" stroke-width="1.2"/>
                        <path d="M5 10a7 7 0 0014 0M12 19v3M8 22h8" stroke="#ef4444" stroke-width="1.5" stroke-linecap="round"/>
                    </svg>
                </div>
                ...
                <div class="rekam-sukses">
                    <div class="rekam-sukses-icon">
                        <svg width="26" height="26" fill="none" viewBox="0 0 24 24">
                            ...
                            <path d="M7 12l4 4 6-6" stroke="#22c55e" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </div>
            ...
            <div class="rekam-btn-group" id="rekam-btn-group">
                <button type="button" class="btn-rekam-mulai" id="rbtn-mulai" onclick="mulaiRekam()">
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24">
                        ...
                        <path d="M5 10a7 7 0 0014 0M12 19v3M8 22h8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
                    </svg>
                    Mulai Rekam
                </button>
                <button type="button" class="btn-rekam-stop" id="rbtn-stop" style="display:none" onclick="stopRekam()">
                    <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
                        <rect x="5" y="5" width="14" height="14" rx="2"/>
                    </svg>
                    Stop
                </button>
                <button type="button" class="btn-rekam-ulang" id="rbtn-ulang" style="display:none" onclick="ulangRekam()">
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24">
                ...
                <button type="button" class="btn-modal-save" id="rbtn-kirim" style="display:none" onclick="kirimReferensi()">
                    <svg width="13" height="13" fill="none" viewBox="0 0 16 16">
                        <path d="M14 2L7 9M14 2l-4 11-3-5-5-3 12-3z" stroke="#fff" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    Kirim Referensi
                </button>
            </div>
...
function ulangRekam() { resetRekam(); }

async function kirimReferensi() {
    if (!audioBlob) { showRekamError('Belum ada rekaman.'); return; }
    ...
    fd.append('audio',     new File([audioBlob], `referensi.${ext}`, { type: audioBlob.type }));
    fd.append('id_materi', currentIdMateri);
    fd.append('_token',    csrfToken);

    try {
        const resp = await fetch(routeReferensi, { method: 'POST', body: fd });
